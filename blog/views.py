...
def index(request):
    posts = Post.objects.prefetch_related('author').annotate(Count('comments', distinct=True))
    most_popular_posts = posts.annotate(num_likes=Count('likes', distinct=True)).order_by('-num_likes')[:5]
